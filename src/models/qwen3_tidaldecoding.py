...
from transformers.models.llama.configuration_llama import LlamaConfig as Qwen3Config
...
def _compute_yarn_parameters_qwen3(config, device, seq_len=None, **rope_kwargs):
    """YaRN matching modern transformers (>=4.51) / official Qwen3 recipe.

    transformers 4.45.1's _compute_yarn_parameters ignores
    `original_max_position_embeddings` and computes the NTK-by-parts correction
    range on config.max_position_embeddings, which diverges from the official
    Qwen3 YaRN setup. This local version computes the ramp on the original
    (pre-scaling) context length, exactly like newer transformers.
    """
...
class Qwen3RotaryEmbedding(nn.Module):
    def __init__(
        ...
        if self.rope_type == "yarn" and config is not None:
            # use official Qwen3 yarn (original_max-based ramp), see _compute_yarn_parameters_qwen3
            self.rope_init_fn = _compute_yarn_parameters_qwen3
...
class Qwen3Attention(nn.Module):
    ...
    def __init__(self, config: Qwen3Config, layer_idx: Optional[int] = None):
        ...
        # Qwen3: per-head RMSNorm on Q and K, applied on head_dim (after projection, before RoPE)
...
class Qwen3Model(Qwen3PreTrainedModel):
    ...
    def forward(
        ...
        for i, decoder_layer in enumerate(self.layers):
            decoder_layer.self_attn.pos_dict = pos_dict
            ...
            pos_dict = decoder_layer.self_attn.pos_dict
...
class Qwen3ForCausalLM(Qwen3PreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        >>> model = Qwen3ForCausalLM.from_pretrained("Qwen/Qwen3-14B")
...
class Qwen3ForQuestionAnswering(Qwen3PreTrainedModel):
    ...
    # Copied from transformers.models.bloom.modeling_bloom.BloomForQuestionAnswering.__init__ with Bloom->Qwen3
    def __init__(self, config):
        super().__init__(config)
